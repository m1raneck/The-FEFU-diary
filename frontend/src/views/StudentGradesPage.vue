  ...
  <div class="screen">
    <div class="grades-card">
      ...
      <div class="header-section">
        ...
        <p v-if="groupName" class="group-label">Группа: {{ groupName }}</p>
      </div>
        ...
        <div v-for="subj in subjects" :key="subj.id" class="subject-card">
          ...
          <div class="table-wrapper">
            <table class="grades-table">
              <thead>
                <tr><th>Дата</th><th>Оценка</th><th>Посещение</th></tr>
              </thead>
              <tbody>
                <tr v-for="row in subj.rows" :key="row.date">
                  ...
                  <td class="attendance-cell">
                    <span class="attendance-chip" :class="attendanceChipClass(row.attendance)">
...
import { getStoredUser, logout as authLogout, isStudent } from '@/services/auth'
...
export default {
  ...
  computed: {
    ...
    groupName() {
      return this.user?.student_info?.group_name || ''
    }
  },
  async mounted() {
  ...
  methods: {
    ...
    attendanceChipClass(attendance) {
      if (attendance === 'present') return 'chip-present'
      if (attendance === 'absent') return 'chip-absent'
      return ''
    },
        ...
        this.subjects = Object.entries(bySchedule).map(([scheduleId, data]) => {
          ...
          const rows = dates.map(iso => {
            ...
            return {
              ...
              grade: g?.grade ?? null,
              attendance: a ? (a.status === 'present' || a.status === 'late' ? 'present' : 'absent') : null
            }
          })

          const numericGrades = rows
            .map(r => r.grade)
            .filter(g => g != null && !isNaN(parseFloat(g)) && typeof g !== 'string')
            .map(g => parseFloat(g))
          const sumGrades = numericGrades.reduce((a, b) => a + b, 0)
          const avg = numericGrades.length ? Math.round((sumGrades / numericGrades.length) * 10) / 10 : null

          const known = rows.filter(r => r.attendance !== null)
          const attendancePct = known.length
            ? Math.round((known.filter(r => r.attendance === 'present').length / known.length) * 100)
            : 0
