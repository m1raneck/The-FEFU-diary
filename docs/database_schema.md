...
- Один студент учится в одной группе (`students.group_id` → `groups.id`).
- Один преподаватель ведёт много предметов (`teacher_subjects`).
- Одна группа имеет много записей в расписании (`schedule`).
- Один студент имеет много оценок (`grades`) и записей посещаемости (`attendance`).
- Одно занятие (`schedule`) может иметь много оценок и записей посещаемости.